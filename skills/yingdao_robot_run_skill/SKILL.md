...
- **YINGDAO_USER_PATH**: Open ShadowBot → Right-click any robot → Open folder location → Navigate up to `users\数字ID` directory
...
4. Uses `GetExitCodeProcess` (Win32 API) to verify whether each engine process is still alive (STILL_ACTIVE=259)
...
4. 通过 `GetExitCodeProcess`（Win32 API）验证每个 engine 进程是否仍存活（STILL_ACTIVE=259）
